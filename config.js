...
function fetchMachineConfigs() {
  const sql = 'select fingerprint, name, data ' + 'from devices left join machine_configs ' + 'on devices.fingerprint=machine_configs.device_fingerprint ' + 'where devices.authorized=$1';

  return fetchGlobalSettings().then(globalSettings => {
    return db.manyOrNone(sql, true).then(rows => rows.map(R.merge(globalSettings)));
  });
}
...
function fetchGlobalSettings() {
  return Promise.all([fetchGlobalConfig(), fetchGlobalSchema()]).then(rs => ({ config: rs[0].exchanges.settings, schema: rs[1] }));
}

function fetchCryptoConfig() {
  return db.oneOrNone('select data from user_config where type=$1', ['crypto']).then(row => row ? row.data : {});
}
...
function fetchCryptoSettings() {
  return Promise.all([fetchGlobalConfig(), fetchCryptoConfig(), fetchCryptoSchema()]).then(rs => ({
...
function fetchMachineConfig(machineId) {
  ...
  return db.one(sql, machineId).then(row => row.data);
}

function updateMachineConfig(machineId, settings) {
  return fetchMachineConfig(machineId).then(oldSettings => {
    if (!oldSettings) {
...
function updateGlobalConfig(settings) {
  return fetchGlobalConfig().then(config => {
    ...
    return db.none('update user_config set data=$1 where type=$2', [newConfig, 'exchanges']).then(() => fetchGlobalSettings());
  });
}
