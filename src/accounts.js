...
import type {AccountCommand} from '../../types/global'
...
function getSchema (account) {
  ...
  return fsReadFile(schemaPath)
  .then(JSON.parse)
  .catch(e => {
    if (e.code === 'ENOENT') {
      return fsAccess(accountPath)
      .then(() => { throw new Error('Account has no schema file') })
...
function getAccountData (account) {
  ...
  .then(config => config.exchanges.plugins.settings[account])
}
...
function getAccount (account) {
  return getSchema(account)
  .then(schema => {
    return getAccountData(account)
    .then(accountData => ({schema: schema, accountData: accountData}))
    .then(elideSecrets)
  })
}

function getAccountList () {
  return fsReaddir(PLUGIN_PATH)
  .then(dirs => {
    const schemaPromises = dirs.map(accountName => {
      ...
      .then(schema => ({code: code, name: schema.jsonSchema.title}))
      .catch(() => undefined)
    })
...
function updateAccount (account, accountData) {
  ...
  .then(config => {
    ...
    .then(accountRec => accountRec.accountData)
  })
}

function getConfiguredAccounts () {
  return fetchUserConfig()
  .then(config => R.keys(config.exchanges.plugins.settings))
  .then(configuredAccounts => {
    return getAccountList()
    .then(R.filter(r => R.contains(r.code, configuredAccounts)))
  })
}

function handle (req: {command: AccountCommand; account: string; accountData: {}}) {
  switch (req.command) {
    case 'account':
